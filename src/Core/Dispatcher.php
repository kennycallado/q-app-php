  ...
  private function action(string $method, string $route, ?array $params = null)
  {
    ...
    if (
      ...
      if (!isset($_COOKIE["gAuth"])) {
        ...
        if (isset($_COOKIE["iAuth"])) { $auth->iAuth = $_COOKIE["iAuth"]; }
        if (isset($_COOKIE["project"])) { $auth->project = json_decode($_COOKIE["project"]); }
    ...
    if ($this->is_protected) {
      if ($body === null) $controller->$action($auth, $params);
      else $controller->$action($auth, $body, $params);
    } else {
      if ($body === null) $controller->$action($params);
      else $controller->$action($body, $params);
    }
